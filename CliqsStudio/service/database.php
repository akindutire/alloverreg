<?php
	...
	class CQS_Database extends CQS_Config{
		...
		private function Connection($con_params){
			...
			/****
			*	More Development is Needed in Future -Supported Database , [Mysql,Sqlite]
			*/
			...
			try {
				...
				if ($con_params['driver'] == 'mysql') {
					...
					$connect_handle = new PDO("$DatabaseType:host=$Host;dbname=$DatabaseName", "$DatabaseUsername", "$DatabasePassword");
		...
		public final function addConnection($con_params = []){
			...
			$Logger = new CQS_Logger;
			$Logger->checkLive('Connection Established');
